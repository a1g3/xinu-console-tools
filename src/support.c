 ...
 */
struct condata *
getcondata( struct condata *plist, char *pname, char *pclass )
{
	for( ; plist != NULL; plist = plist->next ) {
		if( strnequ( plist->conname, pname, MAXCONNECTIONNAME ) ) {
			if( pclass != NULL && *pclass != '\0' ) {
				if( strnequ( plist->conclass, pclass, MAXCLASSNAME ) ) {
					return( plist );
				}
			}
			else {
 ...
 */
struct condata *
readConfigurationFile( char *filename )
{
 ...
 */
static char *
matchtoken( int tk, char *filename )
{
	if( tk != tknext() ) {
