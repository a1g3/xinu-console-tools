 ...
 *----------------------------------------------------------------------------
 */
int connectUDP(char *host, int service)
 ...
 *----------------------------------------------------------------------------
 */
int connectTCP(char *host, int service)
/*     char *host;	     name of host to which connection is desired  */
/*     int service;	     service associated with desired port	  */
{
 ...
 *---------------------------------------------------------------------------
 */
char *sockgetstr(int s, char *str, int timeout)
{
	char *sb;
 ...
 *----------------------------------------------------------------------------
 */
int connectsock(char *host, int service, char *protocol)
/*      char *host;	        name of host to which connection is desired  */
/*      int   service;	        service associated with desired port	     */
/*      char *protocol;	        name of protocol to use ("tcp" or "udp" )    */
{
	...
	if( ( s = socket( PF_INET, type, ppe->p_proto ) ) < 0 ) {
	  fprintf( stderr,
		   "connectsock(%s, %d, %s): can't get create socket: %s\n", 
 ...
 *----------------------------------------------------------------------------
 */
int passivesock(int service, char *protocol, int qlen)
/*     int   service;	    service associated with desired port	 */
/*     char *protocol;	    name of protocol to use ("tcp" or "udp" )    */
/*     int   qlen;                                                       */
{
	...
	if( s < 0 ) {
	  fprintf( stderr, 
		   "passivesock(%d, %s, %d): can't create socket: %s\n", 
 ...
 *---------------------------------------------------------------------------
 */
static int bnets(struct in_addr *addrs, int s)
{
	struct ifconf ifc;
 ...
 *---------------------------------------------------------------------------
 */
int bcastUDP(int s, char *buf, int len, int port)
{
	...
	if( setsockopt( s , SOL_SOCKET , SO_BROADCAST ,
			(char *) & on , sizeof( on ) ) < 0 ) {
		fprintf( stderr, 
			 "broadcast failed setting socket options: %s\n", 
			 strerror(errno) );
		return( -1 );
	}
	nets = bnets( addrs, s );
 ...
 *---------------------------------------------------------------------------
 */
int readDelay(int fd, int timeout)
{
	if( timeout >= 0 ) {
#ifdef linux
		fd_set reads;
 ...
 *---------------------------------------------------------------------------
 */
int writeDelay(int fd, int timeout)
{
	if( timeout >= 0 ) {
#ifdef linux
		fd_set writes;
