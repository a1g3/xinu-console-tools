 ...
 *---------------------------------------------------------------------------
 */
static int sget(char **from, int *dlen, char *to, int max)
{
	int slen;
...
int recvfromReply(int s, struct sockaddr_in *from, 
		  int timeout, struct reply *reply)
{
	int status;
...
int recvReply(int s, int timeout, struct reply *reply)
{
 ...
 *---------------------------------------------------------------------------
 */
int sendtoRequest(int s, struct sockaddr_in *to, struct request *req)
{
...
int sendRequest(int s, struct request *req)
{
...
void initRequest(struct request * req, char code,
		 char *connection, char *class)
{
	if( connection == NULL ) {
		connection = "";
	}
...
void initReply(struct reply *reply, char code, char *host, int numc)
{
	if( host == NULL ) {
		host = "";
	}
 ...
 *----------------------------------------------------------------------
 */
int reqGeneric(char *connection, char *class, char *host, int code,
	       struct reply *reply)
{
	struct request req;
	struct reply rep;
	...
	if( sock >= 0 && strnequ( sockhost, host, MAXHOSTNAME ) ) {
		/* do nothing */
	}
	else {
		if( sock >= 0 ) {
